...
pub fn typst_tests() {
  describe("typst expected tokens snippet validation", [
    ...
    it("validates with night-owl theme", fn() {
      test_helpers.validate_snippet(languages.Typst, "snippet", themes.NightOwl)
    }),
...
pub fn typst_generated_tests() {
  describe("typst expected generated validation", [
    it("generates all patterns correctly", fn() {
      test_helpers.validate_expected_generated(languages.Typst)
    }),
  ])
}

pub fn typst_recursion_tests() {
  describe("typst expected recursion validation", [
    it("processes all patterns correctly", fn() {
      test_helpers.validate_expected_recursion(languages.Typst)
    }),
  ])
}
